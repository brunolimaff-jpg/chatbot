...
const onWelcomeConsent = (dependencies, closingFlow) => async (ctx, { state, fallBack, gotoFlow }) => {
...
const onCaptureName = async (ctx, { state }) => {
    await state.update({ leadName: String(ctx.body ?? '').trim() })
}

const onCaptureObjective = (dependencies, closingFlow) => async (ctx, { state, flowDynamic, gotoFlow }) => {
...
const onCapturePreferredWindow = async (ctx, { state }) => {
    await state.update({ preferredWindow: String(ctx.body ?? '').trim() })
}

const buildIntakePayload = (ctx, state) => ({
    phoneNumber: ctx.from,
...
const onCaptureConsent = (dependencies, closingFlow) => async (ctx, { state, flowDynamic, gotoFlow, fallBack }) => {
    ...
    try {
        const intake = await dependencies.intakeLeadUseCase.execute(buildIntakePayload(ctx, state))
        await dependencies.handoffLeadUseCase.execute({
            leadId: intake.lead.id,
...
const createWelcomeFlow = (dependencies, closingFlow) =>
    addKeyword(['oi', 'ola', 'agendar', 'agenda', 'quero agendar', 'avaliacao'])
        .addAnswer(WELCOME_MESSAGE, { capture: true }, onWelcomeConsent(dependencies, closingFlow))
        .addAnswer('Qual e o seu nome?', { capture: true }, onCaptureName)
        .addAnswer(
            'Qual procedimento ou resultado estetico voce procura hoje?',
            { capture: true },
            onCaptureObjective(dependencies, closingFlow)
        )
        .addAnswer(
            'Qual periodo voce prefere para atendimento? (manha, tarde, noite ou dia/horario especifico)',
            { capture: true },
            onCapturePreferredWindow
        )
        .addAnswer(
            'Voce autoriza o uso desses dados para contato e agendamento? Responda *sim* ou *nao*.',
            { capture: true },
            onCaptureConsent(dependencies, closingFlow)
        )
